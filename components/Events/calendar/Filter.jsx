...
const Filter = () => {
  ...
  const { online, inHouse, eventType, language, eventTypes, languages, venue, venues, categoryId } = calendarData;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const result = await axios.get(`${getServerURL()}/api/events/category-events/${categoryId}?limit=5&startDateTime=${start}T00:00:00.000Z&endDateTime=${end}T00:00:00.000Z${eventTypeParam}${languageParam}${venueParam}${onlineStatusParam}`)
      dispatchCalendarData({type: CALENDAR_ACTIONS.SET_EVENTS, events: result.data.events});
    };
    if (categoryId) fetchData();
  }, [online, inHouse, eventType, language, venue]);
