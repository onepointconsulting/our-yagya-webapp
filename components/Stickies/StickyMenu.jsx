...
const StickyMenu = () => {
  ...
  return (
    <div className={stickyPillMenu ? "fadeInTopMenu opacity-100	z-10 transition duration-150 ease-in-out sticky top-0 z-10 hidden lg:block" : "fadeOutTopMenu hidden"}>
      <div className="flex-row flex-wrap items-center w-full p-2 mx-auto text-center filosofia_regular bg-grayDark md:flex md:justify-around">
        {/* stick menu */}
        <a
          ...
          menus.map((menu, i) => {
            return (
              <div key={i} className="flex items-center justify-around">
                <Link href="/">
                  <a className="px-2 text-sm text-gray-100 lg:text-2xl xl:text-4xl whitespace-nowrap">
                    {menu.title}
                  </a>
                </Link>
