...
export function convertToLocalZone (dateTimeAsString, timeZone) {
  const orig = DateTime.fromISO(dateTimeAsString.replace(/Z$/, ''),
    { zone: timeZone })
  return orig.toLocal().toString().replace(/Z$/, '')
}

export default function findStartAndEndMonth (
  month = new Date().getMonth(),
  year = new Date().getFullYear(),
) {
  const start = DateTime.fromJSDate(new Date(year, month, 1))
  const end = start.endOf('month')
  let startStr = start.toISO()
  let endStr = end.toISO()
