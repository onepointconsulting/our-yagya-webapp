...
export const calendarAdapter = (data) => {
  ...
  if (calendarData.length > 0) {
    const calendarAttributes = calendarData[0]?.attributes;
    const calendarEventTitle = calendarAttributes?.event?.data?.attributes;
    const title = calendarEventTitle.title;
    const startDate = calendarAttributes.startDate;
    return [{
      title: title,
      start: startDate
    }]
