...
export const calendarAdapter = (calendarData) => {
...
export const calendarSpecialDaysAdapter = (calendarData) => {
  if (calendarData?.length > 0) {
    return calendarData.filter(event => event.specialDay).map(event => {
      return {
        title: event.title,
