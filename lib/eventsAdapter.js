...
export const calendarSpecialDaysAdapter = ({events, selectedStartDate, selectedEndDate}) => {
  ...
  if (events?.length > 0) {
    return events.filter(event => {
      ...
      return event.specialDay && selectedStart <= startDate && selectedEnd > startDate
    }).map(event => {
      return {
        title: event.title,
...
export const calendarFooterEventsAdapter = ({events, selectedStartDate, selectedEndDate}) => {
  const selectedStart = DateTime.fromJSDate(selectedStartDate)
  const selectedEnd = DateTime.fromJSDate(selectedEndDate)
  if (events?.length > 0) {
    return events.filter(event => {
