...
export async function eventsLayout({ locale, resolvedUrl }) {
  ...
  const baseEvents = await axios.get(`${getServerURL()}/api/event-categories/event-tree/${locale}`)
  const { data } = baseEvents
  const categories = data.map(eventByCategory => eventByCategory.title)
  return {
    data: {
...
// {
//   data: {
//     pageLayouts: { data: [Array], __typename: 'PageLayoutEntityResponseCollection' },
//     slides: { data: [Array], __typename: 'SlideEntityResponseCollection' },
//     ???? : events data 
//   },
...
async function baseEventsLayout( locale, resolvedUrl, extraQuery ) {
