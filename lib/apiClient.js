...
export async function newsLayout ({ locale, resolvedUrl }) {
  ...
  const promiseAll = await Promise.all([/*parentCategoryPromise,*/ baseLayoutPromise, baseNewsPromise])

  const [/*parentCategory, */baseLayout, baseNews] = promiseAll

  const { data } = baseNews

  console.log('### Some news: ', promiseAll, data)
