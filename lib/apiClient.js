...
export async function eventsLayout ({ locale, resolvedUrl }) {
  ...
  const parentCategory = hasCategory ? await executeGraphQlQuery(
    createEventCategoryQuery(categoryId)) : null
  const baseLayout = await baseEventsLayout(locale, resolvedUrl)
  const baseEvents = await axios.get(
    `${getServerURL()}/api/event-categories/event-tree/${locale}${urlParams}`)
...
export async function eventsBlogLayout ({ locale, resolvedUrl }) {
  const categoryId = idUrlAdapter(resolvedUrl)
  const parentCategory = await executeGraphQlQuery(createEventCategoryQuery(categoryId)) 
  const baseLayout = await baseEventsLayout(locale, resolvedUrl)
  const baseEvents = await axios.get(
    `${getServerURL()}/api/events/category-events/${categoryId}`)
  const { data } = baseEvents
  return {
    data: {
      pageLayouts: baseLayout.data.pageLayouts,
