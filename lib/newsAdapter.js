...
export const singleNewstAdapter = (data) => {
    ...
    if(singleNewstData.length > 0) {
        ...
        const gettingImages = singleNewsAttributes?.images?.data?.[0]?.attributes;
        ...
        const sNewsImageUrl = gettingImages?.url;
        const sNewsImageAlternativeText = gettingImages?.alternativeText;
        const sNewsImageName = gettingImages?.name;
        return {
            sNewsTitle,
            sNewsTimestamp,
            sNewsContent,
